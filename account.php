...
// Format an order ID as a prefixed display string.
...
// Fetch order stats, including kiosk orders matched by mobile number.
$stmt = mysqli_prepare(
    $conn,
    "SELECT
...
// Fetch full order history, including kiosk orders matched by mobile.
$stmt = mysqli_prepare(
    $conn,
...
// Separate orders into online and kiosk groups for sub-tabs.
...
for ($i = 5; $i >= 0; $i--) {
    ...
    $row = mysqli_fetch_assoc(mysqli_prepare_and_execute(
        $conn,
...
// Calculate average order value from qualifying orders.
$avg_order = $stats['qualifying_orders'] > 0
    ? round($stats['total_spent'] / $stats['qualifying_orders'], 2)
    : 0;

// Helper: prepare, bind, and execute a statement in one call.
...
<body class="page-account">
                            ...
                            <div class="ins-row ins-row--bottom">
                                ...
                                <div class="ins-panel ins-panel--top-items">
                                    ...
                                    <?php if (empty($top_items)): ?>
                                        <div class="acct-empty-state"><i class="bi bi-cup-hot"></i>
                                            <p>No orders yet.</p>
                                        </div>
                                    <?php else: ?>
                                ...
                                <div class="ins-panel ins-panel--donut">
                                    ...
                                    <?php if (empty($type_data)): ?>
                                        <div class="acct-empty-state"><i class="bi bi-truck"></i>
                                            <p>No data yet.</p>
                                        </div>
                                    <?php else: ?>
                                ...
                                <div class="ins-panel ins-panel--donut">
                                    ...
                                    <?php if (empty($pay_data)): ?>
                                        <div class="acct-empty-state"><i class="bi bi-credit-card"></i>
                                            <p>No data yet.</p>
                                        </div>
                                    <?php else: ?>
                                        <div class="ins-donut-wrap">
    ...
    <script>
        ...
        function initInsightsCharts() {
            ...
            // Render the spending line chart.
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        tooltip: {
                            ...
                            padding: 10,
                            callbacks: {
                                label: c => `  ${labels[c.dataIndex]}: ${c.parsed}`
                            }
                        }
                    }
                }
            ...
            // Render the order type doughnut chart.
            ...
            if (typeCtx && insTypeLabels.length) {
                ...
                opts.data = {
                    ...
                    datasets: [{
                        data: insTypeData,
                        backgroundColor: ['#5B1312', '#4a8a6f', '#c49a3c'],
                        borderWidth: 0,
                        hoverOffset: 4
                    }]
                };
            ...
            // Render the payment method doughnut chart.
            ...
            if (payCtx && insPayLabels.length) {
                ...
                opts.data = {
                    ...
                    datasets: [{
                        data: insPayData,
                        backgroundColor: ['#5B1312', '#c49a3c', '#1a6ea8', '#2d8a5e', '#8b4b9e'],
                        borderWidth: 0,
                        hoverOffset: 4
                    }]
                };
        ...
        // Update the favorites nav badge count.
        ...
        // Add a favorite item to the cart.
        function favAddToCart(productId, productName) {
            ...
            fetch('php/add_to_cart.php', {
                ...
                .then(d => {
                    if (d.success) {
                        // Check if the item was newly added or its quantity increased.
        ...
        function discardProfileInfo() {
            ...
            Object.keys(_origInfo).forEach(k => {
                if (f[k]) f[k].value = _origInfo[k];
            });
        }
