 ...
 * Handles: full_name, email, mobile_number, password change,
 *          delivery address fields, and optional profile image upload.
 */
...
if ($mobile_number !== '' && !preg_match('/^09\d{9}$/', $mobile_number)) {
...
if (isset($_FILES['avatar']) && $_FILES['avatar']['error'] === UPLOAD_ERR_OK) {
    ...
    if ($_FILES['avatar']['size'] > 2 * 1024 * 1024) {
        echo json_encode(['success' => false, 'message' => 'Profile image must be under 2 MB.']);
        exit();
    }
...
if ($hashed_password !== null && $profile_image !== null) {
    $sql = "UPDATE users SET full_name=?, email=?, mobile_number=?, password=?,
    ...
    mysqli_stmt_bind_param($stmt, 'sssssssssssi',
        $full_name, $email, $mobile_number, $hashed_password,
        $profile_image, $house_unit, $street_name, $barangay,
        $city_municipality, $province, $zip_code, $user_id
    );
} elseif ($hashed_password !== null) {
    $sql = "UPDATE users SET full_name=?, email=?, mobile_number=?, password=?,
...
if (mysqli_stmt_execute($stmt)) {
    /* Update session name for navbar display */
