...
// Admin dashboard — sales overview, trending products, and recent orders.
...
$stats['customers_active'] = (int)mysqli_fetch_assoc(mysqli_query(
  $conn,
  "SELECT COUNT(DISTINCT u.user_id) AS c FROM users u
     JOIN orders o ON u.user_id = o.user_id WHERE u.role = 'customer'"
))['c'];

// Registered customers with no orders
$stats['customers_inactive'] = (int)mysqli_fetch_assoc(mysqli_query(
  $conn,
...
// Total revenue across kiosk and online qualifying orders
$stats['revenue'] = (float)mysqli_fetch_assoc(mysqli_query(
  $conn,
...
// Show newest active products if no order data exists
if (empty($trending)) {
...
// Last 6 orders including kiosk and online
// Check if order_number column exists
$has_order_num = (bool)mysqli_fetch_assoc(mysqli_query(
  $conn,
...
// Month-over-month comparison deltas
$this_month = date('Y-m');
...
<div class="ins-outer-frame">
  ...
  <!-- Analytics, trending, and recent orders -->
